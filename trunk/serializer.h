...
#define SERIALIZER_H
  ...
  public:
    ...
    /** @brief generalized method for serializing a stl deque */
    template<class T> void operator&(std::deque<T>& p_ObjectList)
    {
        unsigned long count = p_ObjectList.size();
        *this & count;
        if(getType() == Serializer::LoaderType) {
            ...
            T obj;
            for(typename std::deque<T>::iterator it = p_ObjectList.begin();
                    it != p_ObjectList.end(); ++it) {
                obj = *it;
                *this & obj;
            }
